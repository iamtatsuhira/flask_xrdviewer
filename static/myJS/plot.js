...
const customButtons = [
    ...
    },
    {
        ...
        icon: icon.normalization,
        click: (gd) => {
            if (!gd.layout.c_normalized){
                // normalization
                if (gd.layout.c_offset){
                    // remove offset
                    const update_data = offsetRemovedData(gd)
                    const update_layout = offsetRemovedLayout()
                    Plotly.update(gd, update_data, update_layout)
                }
                const update_data = normalizeData(gd)
                const update_layout = normalizeLayout()
                Plotly.update(gd, update_data, update_layout)
            } else {
                // reset normalization
                if (gd.layout.c_offset){
                ...
                const update_data = resetNormalizeData(gd)
                const update_layout = resetNormalizeLayout()
                Plotly.update(gd, update_data, update_layout)
            }      
        }
    },
    {
        ...
        click: (gd) => {
            if (!gd.layout.c_offset){
                // set offset
                const offset = prompt('set offset value', '10000')
                const update_data = offsetAppliedData(gd, offset)
                const update_layout = offsetAppliedLayout(offset)
                Plotly.update(gd, update_data, update_layout)
            } else {
                // reset offset
                const update_data = offsetRemovedData(gd)
...
const plotGraph = (data) => {
    ...
    const modebarbuttons = [customButtons,["zoom2d", "pan2d", "autoScale2d", "resetScale2d", "toImage"]]
